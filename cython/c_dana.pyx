...
cdef class Structure:
    ...
    cdef Group _str
    def __init__(self, tau=0.01, rest=0, noise=0, activation=Identity(), n=9):
        self._str = Group(np.zeros(n,dtype=dtype), tau=tau, rest=rest,
...
cdef class Connection:
    ...
    def __init__(self, source, target, weights, shape=np.array([0]), gain=1.0):
        self._active = True
        self._gain = gain
        self._source = source
        self._target = target
    ...
    def propagate(self):
        ...
        for i in range(t_size):
            ...
            for j in range(s_size):
                v += self._source[j] * self._weights[i*s_size+j]
            self._target[i] += v * self._gain

# --- OneToRow ---
cdef class OneToRow(Connection):
    def propagate(self):
        cdef int i,j
        cdef int n_rows = self._shape[0]
        cdef int n_columns = self._shape[1]
        if not self._active: return

        for i in range(n_rows):
            for j in range(n_columns):
                self._target[i*n_columns+j] += self._source[i] * self._weights[i*n_columns+j] * self._gain


# --- OneToColumn ---
cdef class OneToColumn(Connection):
    def propagate(self):
        cdef int i,j
        cdef int n_rows = self._shape[0]
        cdef int n_columns = self._shape[1]
        ...
        for i in range(n_rows):
            for j in range(n_columns):
                self._target[i*n_columns + j] += self._source[j] * self._weights[i*n_columns +j] * self._gain



# --- RowToOne ---
cdef class RowToOne(Connection):
    def propagate(self):
        cdef int i,j
        cdef double v
        cdef int n_rows = self._shape[0]
        ...
        for i in range(n_rows):
            ...
            for j in range(n_columns):
                   v += self._source[i*n_columns+j] * self._weights[i*n_columns+j]
